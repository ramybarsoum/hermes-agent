...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, OpenAI TTS is available through the **[Tool Gateway](/user-guide/features/tool-gateway)** without a separate OpenAI API key. Run `hermes model` or `hermes tools` to enable it.
:::
