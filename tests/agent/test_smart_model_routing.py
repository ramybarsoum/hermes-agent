...
def test_skips_explicit_action_task_even_when_short():
    prompt = "please write a haiku about snow"
    assert choose_cheap_model_route(prompt, _BASE_CONFIG) is None


def test_routes_tool_heavy_action_as_tool_phase_only():
    prompt = "please search the web for latest OpenAI news"
    result = choose_cheap_model_route(prompt, _BASE_CONFIG)
    assert result is not None
    assert result["provider"] == "openrouter"
    assert result["model"] == "google/gemini-2.5-flash"
    assert result["routing_reason"] == "tool_phase_only"
